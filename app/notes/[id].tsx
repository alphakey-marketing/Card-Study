...
export default function NoteDetailScreen() {
  ...
  useEffect(() => {
    (async () => {
      const [note, allNotebooks] = await Promise.all([
        getNote(id),
        getAllNotebooks(),
      ]);
      ...
      if (note) {
        ...
        setTitle(note.title);
        setBlocks(note.content.length > 0 ? note.content : [{ id: Crypto.randomUUID(), type: "text", content: "" }]);
        setTasks(note.tasks);
        setTags(note.tags);
  ...
  const updateTask = (taskId: string, taskTitle: string) => {
    setTasks((prev) =>
      prev.map((t) => (t.id === taskId ? { ...t, title: taskTitle } : t))
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const updated: Note = {
        ...originalNote,
        title: title.trim(),
        content: blocks.filter((b) => b.content.trim() || b.type === "checkbox"),
  ...
  if (loading) {
    return (
      <View style={[styles.container, { justifyContent: "center", alignItems: "center" }]}>
        <Text style={{ color: Colors.textSecondary, fontFamily: "DMSans_500Medium" }}>Loading...</Text>
      </View>
    );
  }
    ...
    <View style={[styles.container, { backgroundColor: Colors.background }]}>
      ...
      >
        ...
        <View style={styles.headerActions}>
          <Pressable onPress={handleTogglePin} hitSlop={12}>
            <Ionicons
        ...
        <View style={styles.metaSection}>
          ...
          {originalNote ? (
            <Text style={styles.dateInfo}>
              Last edited {formatDate(originalNote.updatedAt)}
            </Text>
          ) : null}
